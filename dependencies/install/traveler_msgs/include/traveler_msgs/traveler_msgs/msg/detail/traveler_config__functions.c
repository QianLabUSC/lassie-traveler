  ...
  traveler_msgs__msg__TravelerConfig__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    traveler_msgs__msg__TravelerConfig * data =
      (traveler_msgs__msg__TravelerConfig *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!traveler_msgs__msg__TravelerConfig__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          traveler_msgs__msg__TravelerConfig__fini(&output->data[i]);
        }
        return false;
      }
    }
